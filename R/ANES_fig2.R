...
estimate_model <- function(df, outcome) {
  form <- as.formula(paste(outcome ," ~ ", "white + black + college_grad + female + factor(income) + factor(age_group)"))
...
voteRepublican_logit <- anesRECODE %>% 
  ...
  group_by(year) %>%
  nest() %>%
  mutate(model = map(data, estimate_model, "votedRepublican2P")) # these are logit coeffieices
