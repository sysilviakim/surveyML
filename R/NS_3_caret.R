...
names(fit_rpart$finalModel$variable.importance) <-
  relaber(names(fit_rpart$finalModel$variable.importance))
dotchart(
  fit_rpart$finalModel$variable.importance[15:1],
  pch = 16, xlab = "Variable importance", main = "Top 15 features"
)
dev.copy(
  pdf, "NS_rpart_trump16_tuned_VIP_july25_TT.pdf",
  width = 10, height = 6
)
dev.off()
...
dev.copy(
  pdf, "output/Nationscape/NS_R2_rpart_biden20_tuned_oct5.pdf",
  width = 10, height = 6
)
dev.off()

names(Biden_rpart$finalModel$variable.importance) <-
  relaber(names(Biden_rpart$finalModel$variable.importance))
names(Biden_rpart$finalModel$variable.importance) <-
  relab2(names(Biden_rpart$finalModel$variable.importance))
dotchart(
  Biden_rpart$finalModel$variable.importance[15:1],
  pch = 16, xlab = "Variable importance", main = "Top 15 features"
)
...
#
# fit_ranger_withImp <- train(
#   trump16 ~ .,
#   data = Xm,
#   subset = Xm_trainset_indices,
#   method = "ranger",
#   importance = "permutation",
#   trControl = fit_control_basic, 
#   # This was previous excecuted with fit_control_basic
#   tuneLength = 10,
# )
#
...
# pred_df_ranger <- cbind(
#   pred_data_ranger$predictions, actual_vote = Xm_testset_data[,"trump16"]
# )
#
...
#
#
# pred_data_caret <- predict(
#   fit_rpart$finalModel, newdata = Xm_testset_data,type="class"
# )
# pred_df_caret <- cbind(
#   pred_data_caret,actual_Vote=Xm_testset_data[,"trump16"]
# )
# # Basic confusion matrix
# table(pred_df_caret)
# # Rich statistics
...
#   labs(
#     y = "Increase in mean squared error in the event of variable permutation",
#     x = "Inputs into the random forest"
#   )
