...
ranger_wImp_spec5_symbolic_ideo <- train(
...
saveRDS(
  ranger_wImp_spec5_symbolic_ideo,
  "CLUSTER_Nationscape_Biden_R2_spec5_SYMBOLILC_MARCH-to-JUNE2020v2.RDS"
)
...
a_analysis[
  , which(names(a_analysis) == "pid7_legacy"):which(names(a_analysis) == "sen_knowledge")
] <- lapply(
  a_analysis[
    , which(names(a_analysis) == "pid7_legacy"):which(names(a_analysis) == "sen_knowledge")
  ],
...
ranger_wImp_spec5_operational_ideo <- train(
  biden20 ~ educ_category_1 +
    educ_category_2 +
    educ_category_3 +
    educ_category_4 +
...
saveRDS(
  ranger_wImp_spec5_operational_ideo,
  "CLUSTER_Nationscape_Biden_R2_spec5_OPERATIONAL_MARCH-to-JUNE2020v2.RDS"
)
