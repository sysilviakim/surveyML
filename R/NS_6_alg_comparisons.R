...
ENS_ROC %>%
  mutate(models = factor(models, levels = c(
    "CART",
    "LASSO",
    "Logit",
    "Random Forest"
  ...
  labs(
    x = "ROC range", y = "Model",
    title = "ROC min to max range",
    subtitle = "The dot in each row shows the mean ROC"
  ) +
...
mod5_spec4_FULL$RF$results %>%
  ...
  geom_errorbar(aes(ymin = L, ymax = U)) +
  labs(
    x = "Number of features available at each split",
...
modelCor(resamples(mod5_spec4_FULL)) %>%
  corrplot::corrplot(
    method = "number", type = "upper", cl.ratio = .4, cl.align = "c"
  )
dev.copy(pdf, "fig/Nationscape/alg_correlations.pdf")
dev.off()

# Inspect how models were tuned;
# mtry & min.node.size make very little difference for RF
mod5_spec4_FULL$RF$results
