library(tidyverse)
library(ggpubr) # ggdensity and ggarrange;
...
# Read in the models:
mod5_spec1_FULL <- readRDS(
  "output/Nationscape/spec1_nationscape_FULLtestsample_Aug4.RDS"
)
mod5_spec2_FULL <- readRDS(
  "output/Nationscape/spec2
  _nationscape_FULLtestsample_Aug4.RDS"
)
mod5_spec3_FULL <- readRDS(
  "output/Nationscape/spec3_nationscape_FULLtestsample_Aug4.RDS"
)
mod5_spec4_FULL <- readRDS(
...
vip(mod5_spec4_FULL$RF$finalModel,
  num_features = 10
) + labs(title = "RF-based variable importance (Nationscape)")
...
# pred_RF_holdoutset <- predict(
#   mod5_spec4_FULL$RF$finalModel,
...
# pred_RF_df <- cbind(
#   pred_RF_holdoutset$predictions,
#   actual_vote = Xm[-Xm_trainset_indices, "trump16"]
# )
# pred_RF_df$predicted_vote <- ifelse(
#   pred_RF_holdoutset$predictions[, 2] >= pred_RF_holdoutset$predictions[, 1],
#   "Trump", "Clinton"
# )

Xm_testset_data$predicted_vote <- factor(
  ifelse(pred_RF_holdoutset$predictions[, 2] >=
...
confusionMatrix(
  factor(Xm_testset_data$predicted_vote), factor(Xm_testset_data$trump16),
  positive = "Trump"
)
...
(Xm_testset_data %>%
  filter(Men == 1) %>%
  mutate(wnc = case_when(
    white_noncollege == 0 ~ "White college educated men",
    white_noncollege == 1 ~ "White non-college men"
  )) %>%
  mutate(jwa = case_when(
    Jwall_agree == 0 ~ "Oppose building the wall",
  ...
  mutate(pid = case_when(
    (pid7_1 == 1 | pid7_2 == 1 | pid7_3 == 1) ~ "Democrats",
    pid7_4 == 1 ~ "Independents",
    (pid7_5 == 1 | pid7_6 == 1 | pid7_7 == 1) ~ "Republicans"
  )) %>%
  ggplot(aes(x = `Prob(Trump)`)) +
  geom_histogram(
    bins = 100,
    aes(y = ..density..)
  ) +
  labs(
    ...
    subtitle = paste0(
      "Predicted probabilities for 16,264 respondents ",
      "in the holdout set (Nationscape)"
    )
  ) +
...
pred_RF_holdoutset1 <- predict(
  mod5_spec1_FULL$RF$finalModel,
  data = Xm_testset_data
)
pred_RF_holdoutset2 <- predict(
  mod5_spec2_FULL$RF$finalModel,
  data = Xm_testset_data
)
pred_RF_holdoutset3 <- predict(
  mod5_spec3_FULL$RF$finalModel,
  data = Xm_testset_data
)

# RF_predictions_4specifications <- data.frame(
#   actual_vote = Xm_testset_data$trump16,
...
RF_predictions_4specifications <- read_csv(
  "output/Nationscape/RF_predictions_4specifications_FULL_Aug5.csv"
)

CM_spec1 <- confusionMatrix(
  factor(RF_predictions_4specifications$spec1_trump >= .5),
  factor(Xm_testset_data$trump16 == "Trump")
)
CM_spec2 <- confusionMatrix(
  factor(RF_predictions_4specifications$spec2_trump >= .5),
  factor(Xm_testset_data$trump16 == "Trump")
)
CM_spec3 <- confusionMatrix(
  factor(RF_predictions_4specifications$spec3_trump >= .5),
  factor(Xm_testset_data$trump16 == "Trump")
)
CM_spec4 <- confusionMatrix(
  factor(RF_predictions_4specifications$spec4_trump >= .5),
  factor(Xm_testset_data$trump16 == "Trump")
)

main_results_NS <- data.frame(
  spec =
    c(
...
main_results_NS %>%
  ...
  geom_point(
    size = 1.25, shape = 21, stroke = 0.4, color = "black",
    fill = "black", shape = 21
  ) +
  coord_cartesian(xlim = c(.6, .95)) +
  labs(
    x = "Accuracy (with 95% CI)", y = "Model features",
...
hexbinplot(
  pred_RF_holdoutset$predictions[, 2] ~ pred_RF_holdoutset3$predictions[, 2],
  ...
  border = gray(.85),
  main = paste0(
    "Correlation between the predicted probability of Trump vote\n",
...
Xm_testset_data %>%
  filter(pid7_5 == 1 | pid7_6 == 1 | pid7_7 == 1) %>%
  select(predicted_vote, trump16) %>%
  table() %>%
  as_tibble() %>%
  rename(N = n) -> repub_cm
plot_confusion_matrix(repub_cm,
  ...
  ggplot2::labs(
    title = "Republicans (incl. leaners)",
    x = "Actual vote"
  ) -> REPS
...
Xm_testset_data %>%
  filter(pid7_1 == 1 | pid7_2 == 1 | pid7_3 == 1) %>%
  select(predicted_vote, trump16) %>%
  table() %>%
  as_tibble() %>%
  rename(N = n) -> dem_cm
