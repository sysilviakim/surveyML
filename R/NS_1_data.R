# Purpose: Prepare dataset by selecting sensible features, one-hot encoding factors, etc.
# ALSO: Keep only respondents who votes for either Trump or Clinton, for the 2016 analysis
...
# For sub-group stratified forest analyses, you can follow https://drsimonj.svbtle.com/running-a-model-on-separate-groups
# Note on select() https://gist.github.com/djhocking/62c76e63543ba9e94ebe

a <- read_dta("~/Dropbox/surveyML/data/nationscape/nationscape_CD_merged.dta")
...
## [here we omit news_consumption]
a_analysis <- a %>% filter(vote_2016 <=2) %>%   # Only Clinton and Trump voters
  select(outcome,           # Select potential model  features
         gender_vars,
         racial_att_vars,
         group_attitudes,
...
Xm$trump16 <- factor(Xm$trump16, labels = c("Clinton", "Trump"))
